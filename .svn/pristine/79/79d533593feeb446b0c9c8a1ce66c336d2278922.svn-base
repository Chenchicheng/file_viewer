...
import com.jjshome.util.FileCharsetDetector;
    ...
    public void conventer(FileModel fileModel) {
        ...
        if (hashDir.exists() && hashDir.isDirectory()) {
            ...
            if (resourceDir.exists() && resourceDir.isDirectory()) {
                // 纯文本,涉及到文件编码问题
                try {
                    File original = new File(fileModel.getTempDir() + "\\"
                    ...
                    String outFile = resourceDirPath + "\\"
                            + FileUtil.getFileName(fileModel.getOriginalFile())
                            + "-utf8.txt";
                    String inFile = fileModel.getTempDir() + "\\"
                            + fileModel.getOriginalFile();
                    // 将文本文件复制到resource目录
